...
//	}
	...
	override fun getModels(
		...
		if (objectIds == null) models[DbNull.value] = MsServerRoleModel()
		...
		invokeComposite(
			...
			Consumer {
				...
				for (modelEntry in models) {
					...
					model.role = ModelModification(roleMap[modelId], null)
					model.builtin = builtin
					model.serverPermissions = serverPermissions
					model.securables = securables
					model.memberships = membershipMap[modelId]!!
					model.members = memberMap[modelId]!!
				}
